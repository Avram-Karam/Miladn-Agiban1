...
function doPost(e) {
  try {
    ...
    // ── عمليات الدرجات العامة: المستخدم يسجل محاولته مرة واحدة فقط ──
...
function ensureNewScoreSheets(ss) {
  ...
  if (!gs) { gs = ss.insertSheet('GameScores'); gs.appendRow(gameHeaders); formatHeaderRow(gs); }
  let is = ss.getSheetByName('IndividualScores');
  if (!is) { is = ss.insertSheet('IndividualScores'); is.appendRow(individualHeaders); formatHeaderRow(is); }
  let cs = ss.getSheetByName('SiteConfig');
  if (!cs) { cs = ss.insertSheet('SiteConfig'); cs.appendRow(configHeaders); formatHeaderRow(cs); }
  return {gs,is,cs};
}
...
function handleSaveGameAttempt(sheet, body) {
  ...
  const max=maxMap[gameId]; const score=Math.max(0,Math.min(max,Number(body.score||0)));
...
function handleSaveIndividualScore(sheet, body) {
  ...
  const max=category==='pamphlet'?50:allowed[category];
  const score=Math.max(0,Math.min(max,Number(body.score||0)));
  const ss=sheet.getParent(); const {is}=ensureNewScoreSheets(ss);
  const rows=is.getDataRange().getValues();
  for(let i=1;i<rows.length;i++) {
    if(String(rows[i][0]||'').trim().toLowerCase()===name.toLowerCase() && String(rows[i][2]||'').trim()===category) {
      return corsOutput({status:'already_submitted',message:'تم تسجيل الدرجة دي لهذا الشخص بالفعل، ولا يمكن إعادة المحاولة.',score:Number(rows[i][3]||0),max});
    }
  }
...
function handleAdminSetScore(sheet, body) {
  ...
  if(type==='individual') {
    ...
    for(let i=1;i<rows.length;i++) if(String(rows[i][0]||'').trim().toLowerCase()===name.toLowerCase() && String(rows[i][2]||'').trim()===category){row=i+1;break;}
