...
function doPost(e) {
  try {
    ...
    // ── getScorebook: قراءة سجل الدرجات والإعدادات — بدون توكن (قراءة فقط)
    if (action === 'getScorebook') {
      return handleGetScorebook();
    }
    if (action === 'getSiteConfig') {
...
function handleAdminResetQuiz(sheet, body) {
  ...
  const ss=sheet.getParent();
  const {is}=ensureNewScoreSheets(ss);
  const rows=is.getDataRange().getValues();
  for(let i=rows.length-1;i>=1;i--) {
    if(String(rows[i][0]||'').trim().toLowerCase()===name.toLowerCase() &&
       ...
       String(rows[i][2]||'').trim()===category) {
      is.deleteRow(i+1);
      return corsOutput({status:'success',message:'تم إعادة فتح الكويز للطالب. يمكنه المحاولة مرة أخرى.',category});
    }
  }
  return corsOutput({status:'success',message:'لا توجد محاولة مسجلة لهذا الكويز؛ الطالب يستطيع فتحه بالفعل.',category});
}
