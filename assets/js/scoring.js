...
(function(){
  ...
  function profile(){
    try { return JSON.parse(localStorage.getItem('yc2_user_profile') || 'null'); } catch(e){ return null; }
  }
  ...
  async function fetchScorebook(){
    ...
    const result = await DataService.sendToGAS({action:'getScorebook'});
  ...
  async function submitIndividual(category, score, max){
    ...
    const result=await DataService.sendToGAS({action:'saveIndividualScore',name,group,category,score:Math.min(max,Math.max(0,Number(score||0))),max});
    return result;
  }

  window.YCScoring={MAX,GAME_MAX,GAME_LABELS,ATTENDANCE_LABELS,profile,personName,groupName,fetchScorebook,computeGroupSummary,submitGame,submitIndividual,esc};
})();
