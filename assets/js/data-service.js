...
class DataService {
    ...
    static async _loadFromGAS() {
        ...
        // بيانات المشاركين الحية من Google Sheets هي المصدر الأساسي.
        // نحتفظ بالبيانات المحلية كـ fallback إذا تعذر الاتصال.
        ...
        try {
            ...
            gasError = err;
            console.warn('DataService: تعذر تحميل المشاركين من Google Sheets:', err.message);
        }

        structData._gasConnected = gasLoaded;
        structData._gasError = gasLoaded ? null : (gasError ? gasError.message : 'تعذر الاتصال بـ Google Sheets');
    ...
    static async getScorebook() {
        ...
        if (!target) return { gameScores:[], individualScores:[], config:{} };
        try {
            const sep = target.includes('?') ? '&' : '?';
            const res = await fetch(target + sep + 'action=getScorebook', { method:'GET', cache:'no-store' });
            if (!res.ok) throw new Error('HTTP '+res.status);
            const json = await res.json();
            if (json && json.status === 'success') return json.data || {gameScores:[],individualScores:[],config:{}};
        } catch(e) { console.warn('DataService.getScorebook:', e.message); }
        return { gameScores:[], individualScores:[], config:{} };
    }
